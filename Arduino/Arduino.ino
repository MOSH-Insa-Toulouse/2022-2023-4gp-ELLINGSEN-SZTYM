...
void loop() {
  ...
  sensorValue_ard = analogRead(A1);
  sensorValue=sensorValue_ard*5/1024; //Tension sur 5 volt et 10 bits
...
if (abs(millis()-temps_ref) > Periode)
 { 
  temps_ref=millis();
 byte V = map(sensorValue_ard,0,1023,0,255);
 mySerial.write(V);
 }
